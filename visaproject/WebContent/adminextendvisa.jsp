...
<body>
	...
	<div style="margin-top: 5px;margin-left: 53px">
		...
		<table class="table2" >
			<tr class="tr1">
				...
				<th id="th1">Verify Extend</th>
				<th id="th1">Extend Days</th>
			</tr>
	
			<%
				AdminExtendVisa extend = new AdminExtendVisa();
				ArrayList<Extend> arrayList = extend.getExtends();
	
				for (Extend extendz : arrayList) {
			%>
			<tr class="tr1">
				<td id="td1"><%=extendz.getId() %></td>
				<td id="td1"><%=extendz.getUser() %></td>
				<td id="td1"><%=extendz.getCountry() %></td>
				<td id="td1"><%=extendz.getVvtype() %></td>
				<td id="td1"><%=extendz.getDays() %></td>
				<td id="td1"><%=extendz.getCtotal() %></td>
				<td id="td1"><%=extendz.getIvisa() %></td>
				<td id="td1"><%=extendz.getExvisa() %></td>
				<td id="td1"><%=extendz.getEextvisa() %></td>
				<td id="td1"><%=extendz.getExtday() %></td>
			</tr>
			<%
				}
			%>
	...
	<div style="position: absolute;top: 620px;left: 290px">
	
		<form method="POST" action="AdminConfirmExtend" onsubmit="return checkForm(this);" >
		
		<table style="width: 750px;height: 500px">
			<tr>
				<td><p style="font-size: 25px;color: white">Select Id :</p></td>
				<td>
					...
					<%
						...
						for (Extend extendz : arrayList) {
					%>
					<option value="<%=extendz.getId() %>" ><%=extendz.getId() %></option> 
					<%
						}
					%></select><td>
