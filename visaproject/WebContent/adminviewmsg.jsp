...
<body>
	...
	<table class="table2" >
			<tr class="tr1">
				...
				<th id="th1">Name</th>
				<th id="th1">Mail</th>
				<th id="th1">Massage</th>
			</tr>
	
		<%
				ViewMsg msg = new ViewMsg();
				ArrayList<Message> arrayList = msg.getMsgs();
				
				for (Message msgs : arrayList) {
			%>
			<tr class="tr1">
				<td id="td1"><%=msgs.getId()%></td>
				<td id="td1"><%=msgs.getName()%></td>
				<td id="td1"><%=msgs.getMail()%></td>
				<td id="td1"><%=msgs.getMsg()%></td>
			</tr>
			<%
				}
			%>
	...
	<div style="position: absolute;top: 620px;left: 285px">
	
		<form method="POST" action="DeleteMsg" >
		
		
		
		<table style="width: 750px;height: 500px;text-align: center">
			<tr>
				<td><p style="font-size: 25px;color: white">Message Id :</p></td>
				<td>
					...
					<%
 						for (Message msgs : arrayList) {
 					%>
					<option value="<%=msgs.getId() %>" ><%=msgs.getId() %></option> 
					<%
						}
					%></select><td>
			
			<td><p><input id="button" style="background-color: #008CBA" type="submit" value="Delete" ></p></td>
