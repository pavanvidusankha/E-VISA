...
<body>
	...
	<table class="table2" >
			<tr class="tr1">
				...
				<th id="th1">Days</th>
				<th id="th1">Extend Days</th>
				<th id="th1">Charge Per Day</th>
				<th id="th1">Already Amount</th>
			...
			<tr class="tr1">
				...
				<td id="td1">${sessionScope.username }</td>
				<td id="td1">${sessionScope.country }</td>
				<td id="td1">${sessionScope.vtype }</td>
				<td id="td1">${sessionScope.days }</td>
				<td id="td1">${sessionScope.extday }</td>
				<td id="td1">${sessionScope.dcharge }</td>
				<td id="td1">${sessionScope.amount }</td>
				<td id="td1">${sessionScope.total }</td>
			</tr>
	...
	<div style="margin-left:550px;margin-top:400px">
	
		<h3 style="color : white;margin-left:10px;font-size:33px">Total Amount</h3>
		<h3 style="display: inline;color : white;font-size:33px">Rs.</h3><h1 style="display: inline;color : white;font-size:55px">${sessionScope.total }</h1>
	
	</div>
	
	<div style="margin-top: -200px;margin-left: 615px">
	
		<form method="POST" action="UpdateExtend" >
		
		<table style="width: 750px;height: 500px">
		
			<input type="hidden" name="id" value="${sessionScope.id }">
			<input type="hidden" name="total" value="${sessionScope.total }">
			<input type="hidden" name="days" value="${sessionScope.days }">
			<input type="hidden" name="amount" value="${sessionScope.amount }">
			<input type="hidden" name="extday" value="${sessionScope.extday }">
