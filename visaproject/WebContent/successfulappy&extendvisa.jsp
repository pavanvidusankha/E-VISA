...
<body>
	...
	<div style="margin-top: 5px;margin-left: 53px">
		...
		<table class="table2" >
			...
			<%
				ExtendVisa extend = new ExtendVisa();
				ArrayList<Payment> arrayList = extend.getPayment((String)session.getAttribute("username"));
	...
	<div style="position: absolute;top: 630px;left: 300px">
		...
		<form method="POST" action="VisaExtend" onsubmit="return checkForm(this);" >
			...
			<tr>
				...
				<td>
					...
					<%
						...
						for (Payment payment : arrayList) {
					%>
					<option value="<%=payment.getId() %>" ><%=payment.getId() %></option> 
					<%
						}
					%></select></td>
					
					<td><p style="font-size: 25px;color: white">Days : </p></td>
					<td><input style="width: 50px;height: 25px;border-radius: 5px;font-size: 18px" type="number" size="6" name="days" min="3" max="99" value="21"></td>
			
			
			<td><p><input id="button" style="background-color: #008CBA;margin-left: 50px" type="submit" value="Extend Visa" ></p></td>
